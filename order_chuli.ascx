<%@ Control Language="C#" AutoEventWireup="true" CodeFile="order_chuli.ascx.cs" Inherits="order_chuli" %>
    ...
    <ItemTemplate>
        <div class="row mb-3">
            ...
            <div class="col-md-9" style="border-bottom: 1px solid #ccc;">
                ...
                <div class="clearfix"></div>
                <asp:Button runat="server" Visible='<%#Eval("是否发货") %>' CssClass="btn btn-success btn-sm float-right" ID="btnFa" Text="现在发货" CommandName="btnFa" CommandArgument='<%#Eval("订单编号") %>' />
                <asp:Button runat="server" CssClass="btn btn-success btn-sm float-right mr-3 disabled" ID="btnDel" Text='<%# DataBinder.Eval(Container.DataItem,"状态") %>' />
